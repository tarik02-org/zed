...
pub(crate) const SEARCH_ON_TYPE_DEBOUNCE: Duration = Duration::from_millis(300);
...
/// The new match ranges are accumulated separately and swapped in only once the stream finishes,
/// so the previous matches (and their highlights) stay on screen while the search runs.
...
/// still match and pruning `stale_paths` that sort before the incoming files. The resulting match
/// ranges are appended to `new_match_ranges`, which the caller swaps into the model once the whole
/// stream is drained.
async fn apply_reused_chunk(
    ...
    new_match_ranges: &mut Vec<Range<Anchor>>,
    cx: &mut AsyncApp,
) -> Option<()> {
